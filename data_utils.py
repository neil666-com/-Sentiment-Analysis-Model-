...
class Tokenizer4Bert:
    ...
    def text_to_sequence(self, text, reverse=False, padding='post', truncating='post'):
        ...
        if reverse:
            sequence = sequence[::-1]
        return pad_and_truncate(sequence, self.max_seq_len, padding=padding, truncating=truncating)
    def encode_bert_tokens(self, sentence, aspect):
        # 拼接格式：[CLS] 句子 [SEP] 方面 [SEP]，和你ABSADataset里的concat格式一致
        tokens_sentence = self.tokenizer.tokenize(sentence)
        tokens_aspect = self.tokenizer.tokenize(aspect)
        # 构造BERT的输入token序列
        tokens = ['[CLS]'] + tokens_sentence + ['[SEP]'] + tokens_aspect + ['[SEP]']
        # 转换为id
        text_bert_indices = self.tokenizer.convert_tokens_to_ids(tokens)
        # 构造segment_ids：[CLS]+句子+[SEP]为0，方面+[SEP]为1（BERT的分段标识）
        bert_segments_ids = [0]*(len(tokens_sentence)+2) + [1]*(len(tokens_aspect)+1)
        # 补全/截断到max_seq_len，和训练时的处理一致
        text_bert_indices = pad_and_truncate(text_bert_indices, self.max_seq_len)
        bert_segments_ids = pad_and_truncate(bert_segments_ids, self.max_seq_len)
